...
export async function updateDb(outputPath = path.resolve('data/exploitdb-web.json')) {
    const response = await fetch(EXPLOIT_DB_FILES_CSV);
    if (!response.ok)
        throw new Error(`Failed to download Exploit-DB CSV: ${response.status} ${response.statusText}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
    updateDb(process.argv[2]).catch((error) => {
        console.error(error instanceof Error ? error.message : error);
        process.exit(1);
    });
}
//# sourceMappingURL=db-update.js.map