...
export function createResult(targetPath, findings) {
    return {
        targetPath,
        generatedAt: new Date().toISOString(),
        findings,
        summary: Object.fromEntries(severities.map((severity) => [severity, findings.filter((f) => f.severity === severity).length]))
    };
}
export function formatReport(result, format) {
    if (format === 'json')
...
function textReport(result) {
    ...
    for (const finding of result.findings) {
        ...
        if (finding.edbId)
            lines.push(`  EDB-ID: ${finding.edbId}`);
        if (finding.patchedVersion)
            lines.push(`  Patched: ${finding.patchedVersion}`);
        if (finding.snippet)
